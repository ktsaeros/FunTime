...
<# =====================================================
   ...
   - Recent power-related events (last 14 days)
   - Current sleep/hibernate settings (seconds→minutes aware)
   - Hibernate availability check
   - Recommendation for your preferred config:
       AC: no sleep (monitor 20m)
       DC: sleep 15m (monitor 5m)
       Hibernate: OFF
   ===================================================== #>
...
if ($schemeGuid) {
  ...
  # 2c. Display raw GUIDs/aliases (handy when debugging)
  $sleepGuidLine = ($sleepSettings | Select-String 'Power Setting GUID').Line
  $hibGuidLine   = ($hibernateSettings | Select-String 'Power Setting GUID').Line
  if ($sleepGuidLine) { Write-Output ("Sleep after RAW: {0}" -f $sleepGuidLine.Trim()) }
  if ($hibGuidLine)   { Write-Output ("Hibernate after RAW: {0}" -f $hibGuidLine.Trim()) }
  ...
  $sleepAChex = ($sleepSettings | Select-String 'Current AC Power Setting Index').Line
  $sleepDChex = ($sleepSettings | Select-String 'Current DC Power Setting Index').Line
  $hibAChex   = ($hibernateSettings | Select-String 'Current AC Power Setting Index').Line
  $hibDChex   = ($hibernateSettings | Select-String 'Current DC Power Setting Index').Line

  if ($sleepAChex) { $sleepAChex = $sleepAChex.Split(':')[-1].Trim() }
  if ($sleepDChex) { $sleepDChex = $sleepDChex.Split(':')[-1].Trim() }
  if ($hibAChex)   { $hibAChex   = $hibAChex.Split(':')[-1].Trim() }
  if ($hibDChex)   { $hibDChex   = $hibDChex.Split(':')[-1].Trim() }

  function HexToMinutes([string]$hex, [string]$unitsName) {
    if (-not $hex) { return $null }
    $val = [convert]::ToInt32($hex,16)
    if ($unitsName -ieq 'Seconds') {
  ...
  # Hibernate indices are generally in minutes; treat them as minutes
  $hibACmin   = if ($hibAChex) { [convert]::ToInt32($hibAChex,16) } else { $null }
  $hibDCmin   = if ($hibDChex) { [convert]::ToInt32($hibDChex,16) } else { $null }

  # PS 5.1-safe printing (no ternary / no ??)
  $acText  = if ($sleepACmin -ne $null) { $sleepACmin } else { 'unknown' }
  $dcText  = if ($sleepDCmin -ne $null) { $sleepDCmin } else { 'unknown' }
  $hacText = if ($hibACmin   -ne $null) { $hibACmin   } else { 'unknown' }
  $hdcText = if ($hibDCmin   -ne $null) { $hibDCmin   } else { 'unknown' }

  Write-Output ("Sleep after (minutes)  AC/DC: {0} / {1}" -f $acText,  $dcText)
  Write-Output ("Hibernate after (min)  AC/DC: {0} / {1}" -f $hacText, $hdcText)
} else {
