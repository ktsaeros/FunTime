...
$apc = Get-WinEvent -FilterHashtable @{ LogName='Application'; ProviderName='APC Data Service'; StartTime=$since } -ErrorAction SilentlyContinue |
  Select-Object TimeCreated, Id, LevelDisplayName, Message
if ($apc) {
...
# Preferred channel:
try {
  $wu += Get-WinEvent -FilterHashtable @{ LogName='Microsoft-Windows-WindowsUpdateClient/Operational'; Id = @(19,20,21,31,34,41); StartTime=$since } -ErrorAction Stop
} catch { }
# Fallback to System log:
try {
  $wu += Get-WinEvent -FilterHashtable @{ LogName='System'; ProviderName='Microsoft-Windows-WindowsUpdateClient'; Id = @(19,20,21,31,34,41); StartTime=$since } -ErrorAction Stop
} catch { }
if ($wu) {
  $wu | Sort-Object TimeCreated | Select-Object TimeCreated, Id, Message | Format-Table -Wrap
} else {
