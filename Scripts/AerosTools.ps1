...
function Get-Users {
    <# 
    .SYNOPSIS
        Lists user profiles, Admin status, and Account Type (Local/Domain).
        Replaces the old simple audit with detailed logic.
    #>
    Write-Host "--- USER PROFILE & ADMIN STATUS AUDIT ---" -ForegroundColor Cyan
    ...
    # 1) Get Machine SID Base (for identifying Local vs Domain accounts)
    try {
        ...
        # Fallback if generic Administrator is renamed/disabled
        $machineSidBase = "S-1-5-21-UNKNOWN" 
    }

    # 2) Get Local Admin Members (Reliable ADSI Method + Net fallback)
    $adminMembers = @()
    try {
      ...
      $grp.Members() | ForEach-Object {
        ...
        $adminMembers += [pscustomobject]@{ Sid=$sid; Class=$member.Class; Name=$member.Name }
      }
    } catch {
      $names = (& net localgroup Administrators) 2>$null | Select-Object -SkipWhile {$_ -notmatch '^-+$'} | Select-Object -Skip 1 | Where-Object { $_ -and $_ -notmatch 'The command completed successfully' }
      foreach ($n in $names) { $adminMembers += [pscustomobject]@{ Sid=$null; Class='unknown'; Name=$n.Trim() } }
    }
    ...
    Get-CimInstance Win32_UserProfile |
      ...
      ForEach-Object {
        ...
        # Determine Type (Local vs Domain)
        $type = if ($sid -like "$machineSidBase-*") { 'Local' } elseif ($IsDomainJoined) { $DomainName } else { 'Workgroup' }

        # Determine Admin Status
        $admin = if ($adminUserSids -contains $sid) { 'Yes' }
                 elseif ($type -ne 'Local' -and $adminGroupSeen -gt 0) { 'Unknown (Nested Group?)' }
                 else { 'No' }
