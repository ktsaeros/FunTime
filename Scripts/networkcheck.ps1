...
# Agent/service context (doesn't define loss/recovery; adds color)
...
# Optional signal: DNS Client op log warnings/errors
...
# Loss triggers: NETLOGON_FAIL or LINK_DOWN or burst of DNS_FAIL/DHCP_ISSUE (>=3 within 10 min)
# Recovery triggers: NETLOGON_OK or GPO_OK or LINK_UP
$losses = $timeline | Where-Object { $_.Type -in 'NETLOGON_FAIL','LINK_DOWN','DNS_FAIL','DHCP_ISSUE' }
$recoveries = $timeline | Where-Object { $_.Type -in 'NETLOGON_OK','GPO_OK','LINK_UP' }

# Precompute rolling counts for DNS/DHCP
...
# --- Output ---
"`n=== Network Loss/Recovery Intervals ($($Start.ToString('yyyy-MM-dd HH:mm')) -> $($End.ToString('yyyy-MM-dd HH:mm'))) ==="
$intervals | Sort-Object Start | Format-Table -Auto

"`n=== Key Event Timeline (trimmed) ==="
$timeline |
  Where-Object { $_.Type -in 'NETLOGON_FAIL','NETLOGON_OK','LINK_DOWN','LINK_UP','DNS_FAIL','DHCP_ISSUE','GPO_OK','AGENT_EVENT' } |
  Select-Object Time, Type, Source, EventId, @{n='Msg';e={($_.Message -replace '.{200}$','...').Substring(0,[math]::Min(200, $_.Message.Length))}} |
  Sort-Object Time |
  Format-Table -Auto

# Save CSV (intervals + timeline) for ticket
$intervals |
  Sort-Object Start |
  Export-Csv -NoTypeInformation -UseCulture -Path $CsvOut
