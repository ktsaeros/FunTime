...
function Service-StopSafe([string]$name) {
  ...
  if ($svc) {
    if ($script:PSCmdlet.ShouldProcess($name, "Stop service")) {
...
function Service-StartSafe([string]$name) {
  $svc = Get-Service -Name $name -ErrorAction SilentlyContinue
  if ($svc) {
    if ($script:PSCmdlet.ShouldProcess($name, "Start service")) {
...
function Remove-PathSafe([string]$path) {
  if (Test-Path $path) {
    if ($script:PSCmdlet.ShouldProcess($path, "Remove")) {
...
function Empty-RecycleBinSafe {
  ...
  if ($script:PSCmdlet.ShouldProcess("Recycle Bin", "Clear on drives: $($drives -join ',')")) {
...
Write-Log ("Params: TargetDrive={0}; Switches: {1}" -f $TargetDrive, ($PSBoundParameters.Keys | Where-Object { $_ -ne 'TargetDrive' } ) -join ', ')
...
# 1) Disable Hibernate
if ($DisableHibernate) {
  if ($script:PSCmdlet.ShouldProcess("Hibernation", "Disable")) {
...
if ($CleanMgr) {
  ...
  if (Test-Path $cm) {
    if ($script:PSCmdlet.ShouldProcess("CleanMgr", "Run /autoclean")) {
...
if ($ClearTemp) {
  ...
  foreach ($path in $tempPaths) {
    if ($script:PSCmdlet.ShouldProcess($path, "Clear contents (hidden+system, recursive)")) {
      Try-Run {
...
if ($RemoveOptionalFeatures) {
  if ($script:PSCmdlet.ShouldProcess("WindowsMediaPlayer", "Disable feature")) {
  ...
  if ($script:PSCmdlet.ShouldProcess("FaxServicesClientPackage", "Disable feature")) {
...
if ($ComponentCleanup) {
  if ($script:PSCmdlet.ShouldProcess("DISM StartComponentCleanup", "Run")) {
...
if ($DeepComponentCleanup) {
  if ($script:PSCmdlet.ShouldProcess("DISM ResetBase", "Run (irreversible)")) {
...
if ($ReportShadowsOnly) {
  if ($script:PSCmdlet.ShouldProcess("VSS ShadowStorage", "Report usage")) {
...
if ($ShrinkShadowStorage) {
  ...
  foreach ($drive in $fixedDrives) {
    ...
    if ($script:PSCmdlet.ShouldProcess($target, $action)) {
      Try-Run {
        $exe  = 'vssadmin.exe'
...
$summary = @()
$summary += "Free space on ${TargetDrive}: was $startFree GB"
$summary += "Ran low disk space triage procedure"
$summary += "Recycle Bin (pre): ~${rbGB} GB (executing user)"
$summary += "Free space on ${TargetDrive}: now $endFree GB  (Δ ${delta} GB)"
