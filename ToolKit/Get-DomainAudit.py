...
def get_hosting_provider(ip_address):
    ...
    if raw:
        ...
        if org_match: provider = org_match.group(1).strip()
        elif net_match: provider = net_match.group(1).strip()
            
        p_up = provider.upper()
        if "GOOGLE" in p_up: provider = "Google Cloud"
        elif "AMAZON" in p_up or "AWS" in p_up: provider = "AWS"
        elif "CLOUDFLARE" in p_up: provider = "Cloudflare"
        elif "MICROSOFT" in p_up: provider = "Microsoft Azure / 365"
        elif "FASTLY" in p_up: provider = "Fastly (CDN)"
        elif "AKAMAI" in p_up: provider = "Akamai (CDN)"
        elif "RIPE" in p_up: provider = "Europe/Global Registry (See IP)"
...
# 1. Check if domain was passed from PowerShell via Arguments
if len(sys.argv) > 1:
    domain = sys.argv[1]
else:
    # 2. If not, prompt the user manually
    print("-" * 60)
...
if ns_raw:
    nsl = ns_raw.lower()
    if "microsoft" in nsl: ns_prov = "M365 (BDM)"
    elif "bluehost" in nsl: ns_prov = "Bluehost"
    elif "cloudflare" in nsl: ns_prov = "Cloudflare"
    elif "google" in nsl: ns_prov = "Google"
    elif "awsdns" in nsl: ns_prov = "AWS Route 53"
    elif "cscdns" in nsl: ns_prov = "CSC Corporate"

mx_prov = "Custom / Other"
if mx_raw:
    mxl = mx_raw.lower()
    if "arsmtp" in mxl: mx_prov = "AppRiver"
    elif "outlook" in mxl: mx_prov = "Direct-to-MS"
    elif "google" in mxl: mx_prov = "Google Workspace"
    elif "mimecast" in mxl: mx_prov = "Mimecast"
