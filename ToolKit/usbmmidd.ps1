<#
    Aeros Group Virtual Display Manager (Stable Rollback v9.1)
...
function Ensure-Persistence {
    ...
    # Mandatory spaces after binPath= and start= are present here
    $createResult = & sc.exe create usbmmidd binPath= "C:\usbmmidd\deviceinstaller64.exe enableidd 1" start= auto 2>&1
    
    Write-Host "   [Display] Activating monitor via service trigger..." -ForegroundColor Gray
...
function Show-DisplayMenu {
    ...
    switch ($vSel) {
        '1' {
            $already = Test-Path "$destFolder\deviceinstaller64.exe"
            Ensure-ToolsInstalled
            Set-Location $destFolder
            .\deviceinstaller64.exe enableidd 0; .\deviceinstaller64.exe enableidd 0
            if ($already) { .\deviceinstaller64.exe enableidd 1 }
        ...
        '4' {
            ...
            if (Test-Path $destFolder) {
                Set-Location $destFolder; .\deviceinstaller64.exe remove usbmmidd
                Set-Location C:\; Remove-Item $destFolder -Recurse -Force
            }
